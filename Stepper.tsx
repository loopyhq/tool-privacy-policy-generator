          ...
          <div className={`footer-container ${footerClassName}`}>
            <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              ...
              <button
                ...
                {...nextButtonProps}
              >
                {isLastStep ? 'Generate Policy →' : nextButtonText}
              </button>
            </div>
          </div>
        )}
...
function SlideTransition({ children, direction, onHeightReady }: any) {
  ...
  return (
    <motion.div
      ...
      style={{ position: 'relative', left: 0, right: 0, top: 0, overflow: 'visible' }}
    >
      {children}
    </motion.div>
  );
    ...
    <motion.div onClick={handleClick} className="step-indicator" style={disableStepIndicators ? { pointerEvents: 'none', opacity: 0.5 } : {}} animate={status} initial={false}>
      <motion.div
        variants={{
          ...
          active: { scale: 1, backgroundColor: '#1e293b', color: '#fafaf9' },
